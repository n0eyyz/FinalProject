...
# @app.post("/ocr-instagram")
# async def ocr_instagram(file: UploadFile = File(...)):
#     """
#     인스타그램 화면 캡처 이미지에서 OCR로 텍스트 추출
#     """
#     # 파일 저장
#     contents = await file.read()
#     temp_filename = "temp_insta_image.jpg"
#     with open(temp_filename, "wb") as f:
#         f.write(contents)
    
#     # OCR
#     results = reader.readtext(temp_filename)
#     texts = [item[1] for item in results if len(item[1]) > 1]  # 1글자 이상만 추출
#     text_all = "\n".join(texts)
    
#     # 파일 삭제
#     os.remove(temp_filename)

#     return JSONResponse({"text": text_all})
